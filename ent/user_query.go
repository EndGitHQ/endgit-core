...
type UserQuery struct {
	...
	withPluginComments *PluginCommentQuery
...
func (_q *UserQuery) QueryPlugins() *PluginQuery {
...
func (_q *UserQuery) QueryReports() *ReportQuery {
...
func (_q *UserQuery) QueryRatings() *RatingQuery {
...
// QueryPluginComments chains the current query on the "pluginComments" edge.
func (_q *UserQuery) QueryPluginComments() *PluginCommentQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPluginComments: _q.withPluginComments.Clone(),
...
func (_q *UserQuery) WithPlugins(opts ...func(*PluginQuery)) *UserQuery {
	query := (&PluginClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithReports(opts ...func(*ReportQuery)) *UserQuery {
	query := (&ReportClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithRatings(opts ...func(*RatingQuery)) *UserQuery {
	query := (&RatingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithPluginComments tells the query-builder to eager-load the nodes that are connected to
// the "pluginComments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithPluginComments(opts ...func(*PluginCommentQuery)) *UserQuery {
	query := (&PluginCommentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPluginComments = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withPluginComments; query != nil {
		if err := _q.loadPluginComments(ctx, query, nodes,
			func(n *User) { n.Edges.PluginComments = []*PluginComment{} },
			func(n *User, e *PluginComment) { n.Edges.PluginComments = append(n.Edges.PluginComments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadPlugins(ctx context.Context, query *PluginQuery, nodes []*User, init func(*User), assign func(*User, *Plugin)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(plugin.FieldAuthorId)
	}
...
func (_q *UserQuery) loadReports(ctx context.Context, query *ReportQuery, nodes []*User, init func(*User), assign func(*User, *Report)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "reporterId" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadRatings(ctx context.Context, query *RatingQuery, nodes []*User, init func(*User), assign func(*User, *Rating)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(rating.FieldUserId)
	}
...
func (_q *UserQuery) loadPluginComments(ctx context.Context, query *PluginCommentQuery, nodes []*User, init func(*User), assign func(*User, *PluginComment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(plugincomment.FieldUserId)
	}
	query.Where(predicate.PluginComment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PluginCommentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
